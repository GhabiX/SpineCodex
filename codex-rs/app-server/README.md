...
- `turn/spineTree/updated` — `{ threadId, turnId, snapshotSeq, activeNodeId, nodes }` when Spine task-tree projection changes. Each node is `{ nodeId, parentId, summary, status, accounting }`, where `status` is `live`, `opened`, `closed`, or `compacted`. `accounting` is projection metadata and may be `null`; when present, `currentNodeContextTokens` is an open-path node's current context delta from its open baseline, `currentNodeContextProblem` reports missing measurement or coordinate mismatch, `closedSourceSuffixTokens` and `closedMemoryContextTokens` are close/next commit-time closed accounting facts, and `memoryOutputTokens` is the provider output-token count for the generated memory body. These values are projection-only counters and are not part of model-visible history.
