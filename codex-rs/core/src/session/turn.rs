...
struct StreamEventTraceEntry {
    sequence: u64,
    event: StreamEventSummary,
    active_item_before: Option<TurnItemSummary>,
    active_item_is_streaming_to_client_before: bool,
}

#[derive(Debug, Clone, Serialize)]
#[serde(tag = "type", rename_all = "snake_case")]
enum StreamEventSummary {
    Created,
    OutputItemAdded {
        item: ResponseItemSummary,
    },
    OutputItemDone {
        item: ResponseItemSummary,
    },
...
#[derive(Debug, Clone, Serialize)]
struct TurnItemSummary {
    kind: &'static str,
    id: String,
}

#[derive(Debug, Serialize)]
struct StreamEventTraceSnapshot<'a> {
    version: u8,
    reason: &'static str,
    thread_id: String,
    turn_id: &'a str,
    model: String,
    provider: String,
    active_item_at_failure: Option<TurnItemSummary>,
    active_item_is_streaming_to_client_at_failure: bool,
    recent_events: Vec<StreamEventTraceEntry>,
}

fn push_stream_event_trace_entry(
    recent_events: &mut VecDeque<StreamEventTraceEntry>,
    sequence: u64,
    event: &ResponseEvent,
    active_item: Option<&TurnItem>,
    active_item_is_streaming_to_client: bool,
) {
    if recent_events.len() == STREAM_EVENT_TRACE_WINDOW {
        recent_events.pop_front();
    }
    recent_events.push_back(StreamEventTraceEntry {
        sequence,
        event: summarize_response_event(event),
        active_item_before: active_item.map(summarize_turn_item),
        active_item_is_streaming_to_client_before: active_item_is_streaming_to_client,
    });
}

fn write_orphan_output_text_delta_trace(
    sess: &Session,
    turn_context: &TurnContext,
    active_item: Option<&TurnItem>,
    active_item_is_streaming_to_client: bool,
    recent_events: &VecDeque<StreamEventTraceEntry>,
) {
    let Some(dir) = sess.services.debug_request_capture_dir.as_ref() else {
        return;
    };
    let trace = StreamEventTraceSnapshot {
        version: 1,
        reason: "output_text_delta_without_active_item",
        thread_id: sess.conversation_id.to_string(),
        turn_id: turn_context.sub_id.as_str(),
        model: turn_context.model_info.slug.clone(),
        provider: turn_context.provider.info().name.clone(),
        active_item_at_failure: active_item.map(summarize_turn_item),
        active_item_is_streaming_to_client_at_failure: active_item_is_streaming_to_client,
        recent_events: recent_events.iter().cloned().collect(),
    };
    match crate::debug_request_capture::write_stream_event_trace(
        dir,
        turn_context.sub_id.as_str(),
        &trace,
    ) {
        Ok(path) => {
            warn!(path = %path.display(), "captured stream event trace before orphan text delta panic");
        }
        Err(err) => {
            warn!("failed to capture stream event trace before orphan text delta panic: {err}");
        }
    }
}

fn summarize_response_event(event: &ResponseEvent) -> StreamEventSummary {
    match event {
        ResponseEvent::Created => StreamEventSummary::Created,
        ResponseEvent::OutputItemAdded(item) => StreamEventSummary::OutputItemAdded {
            item: summarize_response_item(item),
        },
        ResponseEvent::OutputItemDone(item) => StreamEventSummary::OutputItemDone {
            item: summarize_response_item(item),
        },
        ResponseEvent::OutputTextDelta(delta) => StreamEventSummary::OutputTextDelta {
            delta_preview: truncate_for_trace(delta),
        ...
        ResponseEvent::ToolCallInputDelta {
            ...
            call_id,
            delta,
        } => StreamEventSummary::ToolCallInputDelta {
        ...
        ResponseEvent::ReasoningSummaryDelta {
            delta,
            summary_index,
        } => StreamEventSummary::ReasoningSummaryDelta {
        ...
        ResponseEvent::ReasoningContentDelta {
            delta,
            content_index,
        } => StreamEventSummary::ReasoningContentDelta {
            content_index: *content_index,
            delta_preview: truncate_for_trace(delta),
            delta_chars: delta.chars().count(),
        },
        ResponseEvent::ReasoningSummaryPartAdded { summary_index } => {
            StreamEventSummary::ReasoningSummaryPartAdded {
                summary_index: *summary_index,
            }
        }
        ResponseEvent::ServerModel(server_model) => StreamEventSummary::ServerModel {
            server_model: server_model.clone(),
        },
        ResponseEvent::ModelVerifications(verifications) => {
            StreamEventSummary::ModelVerifications {
                count: verifications.len(),
            }
        }
        ResponseEvent::ServerReasoningIncluded(included) => {
            StreamEventSummary::ServerReasoningIncluded {
                included: *included,
            }
        }
        ResponseEvent::Completed {
            ...
            end_turn,
            ..
        } => StreamEventSummary::Completed {
            response_id: response_id.clone(),
            end_turn: *end_turn,
        },
        ResponseEvent::RateLimits(_) => StreamEventSummary::RateLimits,
        ResponseEvent::ModelsEtag(_) => StreamEventSummary::ModelsEtag,
    }
}

fn summarize_response_item(item: &ResponseItem) -> ResponseItemSummary {
    match item {
        ResponseItem::Message {
            ...
            name: None,
            namespace: None,
            phase: phase.as_ref().map(|phase| format!("{phase:?}")),
        },
        ResponseItem::Reasoning { id, .. } => ResponseItemSummary {
            kind: "reasoning",
            id: Some(id.clone()),
            call_id: None,
            role: None,
        ...
        ResponseItem::LocalShellCall { id, call_id, .. } => ResponseItemSummary {
        ...
        ResponseItem::ToolSearchCall { id, call_id, .. } => ResponseItemSummary {
        ...
        ResponseItem::FunctionCallOutput { call_id, .. } => ResponseItemSummary {
        ...
        ResponseItem::CustomToolCallOutput { call_id, name, .. } => ResponseItemSummary {
        ...
        ResponseItem::ToolSearchOutput { call_id, .. } => ResponseItemSummary {
        ...
        ResponseItem::WebSearchCall { id, .. } => ResponseItemSummary {
        ...
        ResponseItem::ImageGenerationCall { id, .. } => ResponseItemSummary {
        ...
        ResponseItem::Compaction { .. } => response_item_summary_without_ids("compaction"),
        ResponseItem::CompactionTrigger => response_item_summary_without_ids("compaction_trigger"),
        ResponseItem::ContextCompaction { .. } => {
            response_item_summary_without_ids("context_compaction")
        }
        ResponseItem::Other => response_item_summary_without_ids("other"),
    }
}

fn response_item_summary_without_ids(kind: &'static str) -> ResponseItemSummary {
    ResponseItemSummary {
        kind,
        id: None,
        call_id: None,
...
fn summarize_turn_item(item: &TurnItem) -> TurnItemSummary {
    let kind = match item {
...
fn truncate_for_trace(value: &str) -> String {
    let mut output = value
        .chars()
        .take(STREAM_EVENT_TRACE_PREVIEW_CHARS)
        .collect::<String>();
    if value.chars().count() > STREAM_EVENT_TRACE_PREVIEW_CHARS {
        output.push_str("...");
    }
    output
}
...
async fn try_run_sampling_request(
    ...
    let mut stream_event_trace_recent: VecDeque<StreamEventTraceEntry> =
        VecDeque::with_capacity(STREAM_EVENT_TRACE_WINDOW);
    let mut stream_event_trace_sequence = 0_u64;
    let outcome: Result<SamplingRequestResult, SamplingRequestError> = loop {
        ...
        stream_event_trace_sequence = stream_event_trace_sequence.saturating_add(1);
        push_stream_event_trace_entry(
            &mut stream_event_trace_recent,
            stream_event_trace_sequence,
            &event,
            active_item.as_ref(),
        ...
        match event {
            ...
            ResponseEvent::OutputTextDelta(delta) => {
                ...
                if let Some(active) = active_item.as_ref() {
                    ...
                    write_orphan_output_text_delta_trace(
                        sess.as_ref(),
                        turn_context.as_ref(),
                        active_item.as_ref(),
                        active_item_is_streaming_to_client,
                        &stream_event_trace_recent,
                    );
